    ...
    public function readVarInto($varstr, $data, $safeRead = false)
    {
        ...
        while (list($k, $sep) = each($m[1])) {
            if ($sep === '.' || $sep === '[' || $sep === '') {
                ...
                if (is_object($data) && ($safeRead === false || isset($data->$m[2][$k]) || is_callable(array(
                    $data,
                    '__get'
                )))) {
                    $data = $data->$m[2][$k];
...
class Dwoo_Security_Policy
{
    ...
    protected $allowedPhpFunctions = array('str_repeat', 'number_format', 'htmlentities', 'htmlspecialchars', 'long2ip', 'strlen', 'list', 'empty', 'count', 'sizeof', 'in_array', 'is_array');
